...
#   Target 2 (R=3): بستن ۵۰٪ از حجم باقی‌مانده + انتقال حد ضرر به نقطه ورود (ریسک‌فری)
#   Target 3 (R=5): بستن ۷۰٪ از حجم باقی‌مانده + شروع تریل‌کردن حد ضرر
#   Target 4 (R=7): بستن کل حجم باقی‌مانده (پایان معامله)
...
# وزن هر مرحله از حجم اصلی پوزیشن (۵۰٪ در T1، ۵۰٪ از باقی‌مانده در T2 => ۲۵٪ کل، و ...)
_W1, _W2, _W3 = 0.5, 0.25, 0.175
_W4 = 1 - _W1 - _W2 - _W3  # 0.075
...
def compute_final_r(trade: Dict[str, Any]) -> float:
    ...
    if not hit["5"]:
        return _W1 * 1 + _W2 * 3 + (1 - _W1 - _W2) * 0.0  # باقی‌مانده در ریسک‌فری بسته شده
    if not hit["7"]:
        return _W1 * 1 + _W2 * 3 + _W3 * 5 + _W4 * 5.0  # باقی‌مانده با تریلینگ‌استاپ، تخمین در حدود ۵R
    return _W1 * 1 + _W2 * 3 + _W3 * 5 + _W4 * 7.0  # کل مسیر تا تارگت ۴ طی شده
...
def check_open_trade(trade: Dict[str, Any], candle: Dict[str, Any], ema7_now: Optional[float]) -> List[Dict[str, Any]]:
    ...
    if side == "BUY":
        if candle["l"] <= sl:
            trade["closed"] = True
            events.append({"type": stop_event_type(), "price": sl})
            return events
...
TARGET_ACTION_LINE = {
    ...
    7: "\n\n🏁 Close the rest of your position — trade complete.",
}
