...
def generate_unique_amount(base_usd, pending):
    """مبلغ رو به عدد صحیح (بدون اعشار) گرد می‌کنه و یک عدد کوچک تصادفی (۱ تا ۴۹ دلار)
    بهش اضافه می‌کنه تا هر درخواست پرداخت مبلغ منحصربه‌فرد داشته باشه (برای تشخیص خودکار)،
    بدون این‌که به رقم اعشار نیاز باشه."""
    base = round(base_usd)
...
def handle_callback(cb, pending, subscribers, discounts, applied, used_tx, referrals):
    ...
    if data.startswith("net:"):
        ...
        edit_message(
            ...
            f"⚠️ Send the <b>exact whole amount</b> shown (a few dollars above the plan "
            f"price is added automatically so your payment is identified instantly — no decimals needed).\n"
...
def try_confirm_payment(p, transfers, subscribers, discounts, used_tx, referrals) -> bool:
    ...
    for tx in transfers:
        ...
        if abs(value_usdt - p["amount"]) < 0.01 and tx["ts"] >= p["created_at"] - 60:
