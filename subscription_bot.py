...
def main_menu_keyboard():
    return [
        ...
        [{"text": "🧮 Calculate My P&L", "callback_data": "menu:pnl"}],
    ]
...
def run_cycle(state, pending, subscribers, discounts, applied, used_tx):
...
def save_all(state, pending, subscribers, discounts, applied, used_tx):
...
def main():
    ...
    while time.time() - start < LOOP_MAX_SECONDS:
        try:
            run_cycle(state, pending, subscribers, discounts, applied, used_tx)
        except Exception as e:
        ...
        if time.time() - last_commit >= GIT_COMMIT_EVERY_SECONDS:
            save_all(state, pending, subscribers, discounts, applied, used_tx)
            git_commit_and_push()
