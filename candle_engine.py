...
#   Target 4 (R=4): بستن کل حجم باقی‌مانده (پایان معامله)
...
def compute_final_r(trade: Dict[str, Any]) -> float:
    ...
    تارگت‌ها الان روی 1R/2R/3R/4R هستن (به‌جای 1/3/5/7 قبلی) تا احتمال واقعی رسیدن بهشون بیشتر باشه."""
    hit = trade["hit"]
    if not hit["1"]:
    ...
    if not hit["4"]:
        return _W1 * 1 + _W2 * 2 + _W3 * 3 + _W4 * 3.0  # باقی‌مانده با تریلینگ‌استاپ، تخمین در حدود ۳R
    return _W1 * 1 + _W2 * 2 + _W3 * 3 + _W4 * 4.0  # کل مسیر تا تارگت ۴ طی شده
