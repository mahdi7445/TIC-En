...
def is_timeframe_due(tf_key: str, now_utc: datetime) -> bool:
    """طراحی‌شده برای ماندن زیر سقف ۸۰۰ درخواست/روز Twelve Data (~۷۴۰/روز جمعاً).
    فرض: هر ۵ دقیقه صدا زده می‌شود."""
    if FORCE_RUN_ALL:
        return True
...
def step_candle_state(state: Dict[str, Any], o: float, h: float, l: float, c: float, open_time: int):
    ...
    if final_bull:
        ...
        sl = min(ema7_i, swing_low)  # فاصله بیشتر و محافظه‌کارانه‌تر از این دو
...
def open_new_trade(signal: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "hit": {"3": False, "5": False, "7": False}, "closed": False,
    }
...
def check_open_trade(trade: Dict[str, Any], candle: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if side == "BUY":
        if candle["l"] <= sl:
            trade["closed"] = True
            events.append({"type": "stop", "price": sl})
            return events
...
def format_entry_message(display: str, tf_label: str, signal: Dict[str, Any], trade: Dict[str, Any]) -> str:
    ...
    targets_lines = "\n".join([f"🎯 {t}R: {trade['entry'] + sign * t * trade['r']:.4f}" for t in RR_TARGETS])
    return (
        f"{arrow} — {display} {tf_label}\n\n"
        f"Entry: <b>{signal['price']:.4f}</b>\n"
        f"❌ Stop: <b>{trade['sl']:.4f}</b>\n"
        f"{targets_lines}\n\n"
        f"{_now_str()}"
        f"{RISK_LINE}"
    )
...
def format_rr_exit_message(display: str, tf_label: str, trade: Dict[str, Any], event: Dict[str, Any], ts: str) -> str:
    direction = "LONG" if trade["side"] == "BUY" else "SHORT"
    return (
        f"✅ EXIT — {display} {tf_label}\n\n"
        f"{event['level']}:R reached on this {direction} trade.\n"
        f"Entry {trade['entry']:.4f}  ·  Stop {trade['sl']:.4f}  ·  Now {event['price']:.4f}\n\n"
        f"{_now_str()}"
        f"{RISK_LINE}"
    )


def format_stop_message(display: str, tf_label: str, trade: Dict[str, Any], event: Dict[str, Any], ts: str) -> str:
    direction = "LONG" if trade["side"] == "BUY" else "SHORT"
    return (
        f"❌ STOP HIT — {display} {tf_label}\n\n"
        f"Stop-loss hit on this {direction} trade.\n"
        f"Entry was {trade['entry']:.4f}  ·  Stop {trade['sl']:.4f}\n\n"
        f"{_now_str()}"
        f"{RISK_LINE}"
    )
...
def process_symbol_timeframe(symbol: str, display: str, tf_key: str, tf_cfg: Dict[str, Any],
    ...
    for k in new_candles:
        if state.get("open_trade"):
            ...
            for ev in events:
                _send_exit(display, tf_key, tf_cfg, state["open_trade"], ev, state["hist"])
...
def _send_exit(display, tf_key, tf_cfg, trade, event, hist):
    title = f"{display} {tf_cfg['label']} · " + ("Stop" if event["type"] == "stop" else f"{event['level']}R Exit")
    chart = build_chart_from_hist(hist, title, trade=trade)
    ts_str = _now_str()
    if event["type"] == "stop":
        msg = format_stop_message(display, tf_cfg["label"], trade, event, ts_str)
    else:
        msg = format_rr_exit_message(display, tf_cfg["label"], trade, event, ts_str)
    if send_photo(msg, chart):
...
def main():
    ...
    due_tfs = [tf for tf in TIMEFRAMES if is_timeframe_due(tf, now_utc)]
